...
export default function Create({ projects = [], categories = [], accounts = [], vendors = [], advances = [] }) {
    ...
    // How an overpayment should be resolved: refunded to a cash box, or credited to the vendor's wallet.
    const [overpaymentAction, setOverpaymentAction] = useState('return');
    ...
    // Balance lookups used to warn before the user submits a payment the source can't actually cover.
    const selectedVendor = useMemo(() => vendorList.find(v => v.id === data.vendor_id), [vendorList, data.vendor_id]);
    const walletBalance = Number(selectedVendor?.wallet_balance || 0);
    const walletInsufficient = data.pay_type === 'wallet' && !!data.vendor_id && paid > walletBalance;

    const selectedAdvance = useMemo(() => advances.find(a => a.user_id === data.advance_user_id), [advances, data.advance_user_id]);
    const advanceBalance = Number(selectedAdvance?.balance || 0);
    const advanceInsufficient = data.pay_type === 'advance' && !!data.advance_user_id && paid > advanceBalance;
    ...
    const selectStyles = {
        ...
        // Rendered via menuPortalTarget below, so this needs to sit above the fixed bottom action bar (z-[999]) too.
        menuPortal: (base) => ({ ...base, zIndex: 9999 })
    };

    // Selects live inside `overflow-hidden` cards and a sticky sidebar, so without portalling to <body>
    // their open menus get clipped by the card edge instead of floating above the page.
    ...
    const handleSubmit = (e) => {
        ...
        if (bill <= 0) {
            return Swal.fire("Required", "Enter a valid total bill amount.", "warning");
        }

        if (isOverpaid) {
            if (overpaymentAction === 'vendor_credit' && !data.vendor_id) {
                return Swal.fire("Vendor Required", "Select a vendor above to credit the overpayment to their wallet, or choose \"Return to Cash Box\" instead.", "warning");
            }
            if (overpaymentAction === 'return' && !data.return_account_id) {
                return Swal.fire("Action Required", `You entered ৳${Number(overpaymentAmount).toLocaleString('en-IN')} extra. Select a Cash Box to return it to, or choose "Credit to Vendor" instead.`, "warning");
            }
        }

        if (data.pay_type === 'wallet') {
            if (!data.vendor_id) {
                return Swal.fire("Vendor Required", "Select a vendor to pay from their wallet.", "warning");
            }
            if (walletInsufficient) {
                return Swal.fire("Insufficient Wallet Balance", `This vendor's wallet only has ৳${walletBalance.toLocaleString('en-IN')} available.`, "warning");
            }
        }

        if (data.pay_type === 'advance') {
            if (!data.advance_user_id) {
                return Swal.fire("User Required", "Select whose advance balance this should be paid from.", "warning");
            }
            if (advanceInsufficient) {
                return Swal.fire("Insufficient Advance Balance", `This user's advance balance is only ৳${advanceBalance.toLocaleString('en-IN')}.`, "warning");
            }
        }

        if (data.pay_type === 'account' && !data.account_id) {
            return Swal.fire("Account Required", "Select an account to pay from.", "warning");
        }

        // Only send return_account_id when it's actually relevant, so a stale value
        // from a previous overpayment / action choice never reaches the backend.
        transform((formData) => ({
            ...formData,
            return_account_id: (isOverpaid && overpaymentAction === 'return') ? formData.return_account_id : '',
        }));
                            ...
                            <div className="p-6">
                                ...
                                {data.pay_type === 'advance' && (
                                    <div>
                                        ...
                                        {advanceInsufficient && <p className="text-rose-500 text-xs mt-1.5 font-medium"><i className="fa-solid fa-triangle-exclamation mr-1"></i>Paid amount exceeds this user's advance balance of ৳{advanceBalance.toLocaleString('en-IN')}.</p>}
                                    </div>
                                )}
                                {data.pay_type === 'wallet' && (
                                    <div className="bg-purple-50 border border-purple-100 rounded-xl p-4 text-center">
                                        {data.vendor_id ? (
                                            <div>
                                                <p className="text-purple-600 font-bold mb-1">Available Vendor Wallet Balance</p>
                                                <h3 className="text-2xl font-black text-purple-800">BDT {walletBalance.toLocaleString('en-IN')}</h3>
                                                {walletInsufficient && <p className="text-rose-500 text-xs mt-2 font-semibold"><i className="fa-solid fa-triangle-exclamation mr-1"></i>Paid amount exceeds this vendor's wallet balance.</p>}
                                            </div>
                                        ) : <p className="text-purple-600 font-medium">Please select a vendor above first.</p>}
                                    </div>
                                )}
                            </div>
                        </section>
                                    ...
                                    <div className="mt-6 pt-5 border-t border-dashed border-amber-500/50">
                                        <div className="flex items-start gap-3 mb-4">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-[12px] text-gray-300 leading-relaxed">You entered <strong>৳{Number(overpaymentAmount).toLocaleString('en-IN')}</strong> extra. Choose how to handle it.</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex gap-2 mb-3">
                                            <button
                                                ...
                                                className={`flex-1 rounded-lg border px-3 py-2 text-[12px] font-bold transition-all ${overpaymentAction === 'return' ? 'border-amber-500 bg-amber-500/10 text-amber-400' : 'border-gray-700 text-gray-400 hover:border-gray-600'}`}
                                            >
                                                Return to Cash Box
                                            </button>
                                            <button
                                                type="button"
                                                onClick={() => { if (data.vendor_id) { setOverpaymentAction('vendor_credit'); setData('return_account_id', ''); } }}
                                                disabled={!data.vendor_id}
                                                title={!data.vendor_id ? 'Select a vendor above first' : ''}
                                                className={`flex-1 rounded-lg border px-3 py-2 text-[12px] font-bold transition-all ${overpaymentAction === 'vendor_credit' ? 'border-amber-500 bg-amber-500/10 text-amber-400' : 'border-gray-700 text-gray-400 hover:border-gray-600'} ${!data.vendor_id ? 'opacity-40 cursor-not-allowed' : ''}`}
                                            >
                                                Credit to Vendor
                                            </button>
                                        </div>
                                        ...
                                        {overpaymentAction === 'return' ? (
                                            <div className="text-gray-900">
                                                <Select options={returnAccountOptions} value={returnAccountOptions.find(o => o.value === data.return_account_id) || null} onChange={opt => setData('return_account_id', opt ? opt.value : '')} placeholder="Select Cash Box..." isClearable styles={selectStyles} menuPortalTarget={selectMenuPortalTarget} />
                                                {errors.return_account_id && <p className="text-rose-400 text-xs mt-1.5 font-medium">{errors.return_account_id}</p>}
                                            </div>
                                        ) : (
                                            <p className="text-[12px] text-gray-300 leading-relaxed bg-gray-800/60 rounded-lg px-3 py-2.5">
                                                <i className="fa-solid fa-circle-info text-amber-400 mr-1.5"></i>
                                                The extra ৳{Number(overpaymentAmount).toLocaleString('en-IN')} will be added to <strong>{selectedVendor?.name || 'the selected vendor'}</strong>'s wallet balance.
                                            </p>
                                        )}
                                    </div>
                                )}
                    ...
                    <div className="fixed bottom-0 left-0 md:left-[270px] right-0 z-[999] bg-white border-t border-gray-200 shadow-[0_-10px_40px_rgba(0,0,0,0.08)]">
                        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-col sm:flex-row justify-between items-center gap-4">
                            ...
                            <div className="flex gap-3 w-full sm:w-auto">
                                ...
                                <button type="submit" disabled={processing || walletInsufficient || advanceInsufficient} className="flex-1 sm:flex-none flex items-center justify-center gap-2.5 bg-indigo-600 text-white px-8 py-3 rounded-xl font-bold text-[14.5px] hover:bg-indigo-700 transition-all shadow-md disabled:opacity-70 disabled:cursor-not-allowed">
