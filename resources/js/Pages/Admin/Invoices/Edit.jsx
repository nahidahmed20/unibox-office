...
export default function Edit({ invoice, clients = [], projects = [] }) {
    ...
    // নতুন লজিক: অন্য লাইনে সিলেক্ট হওয়া প্রজেক্টগুলো বাদ দিয়ে অপশন রিটার্ন করবে
    ...
    return (
        <AdminLayout>
            ...
            <div className="flex items-center justify-between mb-6">
                <div>
                    <h1 className="text-[22px] font-bold text-[#202223]">Edit Invoice #{invoice.invoice_number}</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div className="bg-white p-5 rounded-xl border-l-4 border-blue-500 shadow-sm">
                    ...
                    <h2 className="text-2xl font-black text-gray-900">TK {Number(invoice.grand_total).toLocaleString('en-IN')}</h2>
                </div>
                <div className="bg-white p-5 rounded-xl border-l-4 border-emerald-500 shadow-sm">
                    ...
                    <h2 className="text-2xl font-black text-emerald-700">TK {totalPaidAmount.toLocaleString('en-IN')}</h2>
                </div>
                <div className="bg-white p-5 rounded-xl border-l-4 border-red-500 shadow-sm">
                    <p className="text-xs font-bold text-red-500 uppercase">Total Due</p>
                    <h2 className="text-2xl font-black text-red-600">TK {dueAmount.toLocaleString('en-IN')}</h2>
                </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-200">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8 border-b pb-8">
                    <div className="lg:col-span-2">
                        ...
                        <Select options={clientOptions} value={clientOptions.find(o => o.value === data.client_id)} onChange={(opt) => {
                    ...
                    <div key={index} className="bg-gray-50 p-5 rounded-xl border border-gray-200 mb-4 relative">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-4 gap-5 pr-8">
                            ...
                            <div className="lg:col-span-3">
                                ...
                                <input type="text" value={item.item_name} onChange={(e) => updateItem(index, "item_name", e.target.value)} className="w-full rounded-md border-gray-300" required />
                            ...
                            <div>
                                ...
                                <div className="flex gap-2">
                                    <input type="number" step="any" value={item.quantity} onChange={(e) => updateItem(index, "quantity", e.target.value)} className="w-1/3 rounded-md border-gray-300" />
                                    <input type="number" step="any" value={item.unit_price} onChange={(e) => updateItem(index, "unit_price", e.target.value)} className="w-2/3 rounded-md border-gray-300" />
                                </div>
                            </div>
                            <div>
                ...
                <div className="mt-8 flex justify-end gap-4 border-t pt-6">
                    <button type="submit" disabled={processing} className="bg-[var(--accent)] text-white px-8 py-3 rounded-xl font-bold hover:bg-[#b08630]">{processing ? 'Saving...' : 'Update Invoice'}</button>
                </div>
            </form>
        </AdminLayout>
    );
