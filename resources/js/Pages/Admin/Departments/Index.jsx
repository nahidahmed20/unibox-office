        ...
        printWindow.document.write(`
            <html>
                ...
                <body>
                    ...
                    <h2 class="report-title">Departments Directory</h2>
        ...
        <AdminLayout>
            ...
            <div className="flex flex-col gap-6">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-[22px] font-bold text-[#202223]">Department Workspace</h1>
                        <p className="text-[14px] text-gray-500 mt-1">Manage and track all company departments.</p>
                    </div>
                </div>
                ...
                <div className="rounded-xl border border-[#e1e3e5] bg-white shadow-sm overflow-hidden">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-[#e1e3e5] px-6 py-4 gap-4 bg-gray-50/50">
                        <div className="text-[16px] font-semibold text-[#202223] flex items-center gap-2.5">
                            <i className="fa-solid fa-building text-[var(--accent)]"></i> Department Directory
                        </div>
                            ...
                            <tbody className="text-[13.5px] text-[#202223]">
                                {departments.data && departments.data.length > 0 ? (
                                    departments.data.map((department, index) => (
                                        <tr key={department.id} className="border-b border-gray-100 hover:bg-gray-50/50 transition-colors">
                                            <td className="px-6 py-4 font-medium text-gray-500">
                                                {departments.from ? departments.from + index : index + 1}
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className="font-bold text-gray-900 text-[14px]">{department.name}</div>
                                            </td>
                                            <td className="px-6 py-4">
                                            ...
                                            <td className="px-6 py-4 text-center">
                                                <div className="flex items-center justify-center gap-1.5">
                                                    {hasPermission('view_department') && (
                                                        <button onClick={() => openViewModal(department)} className="flex h-7 w-7 items-center justify-center rounded bg-blue-50 text-blue-600 hover:bg-blue-100 transition-colors" title="View Details">
                                                            <i className="fa-regular fa-eye text-[12px]"></i>
                                                        </button>
                                                    )}
                                                    {hasPermission('edit_department') && (
                                                        <button onClick={() => openEditModal(department)} className="flex h-7 w-7 items-center justify-center rounded bg-gray-100 text-gray-700 hover:bg-gray-200 transition-colors" title="Edit Department">
                                                            <i className="fa-regular fa-pen-to-square text-[12px]"></i>
                                                        </button>
                                                    )}
                                                    {hasPermission('delete_department') && (
                                                        <button onClick={() => handleDelete(department.id)} className="flex h-7 w-7 items-center justify-center rounded bg-red-50 text-red-600 hover:bg-red-100 transition-colors" title="Delete Department">
                                    ...
                                    <tr>
                                        <td colSpan="4" className="px-6 py-12 text-center text-gray-500">
                                            <div className="flex flex-col items-center justify-center">
                                                <i className="fa-solid fa-building text-4xl text-gray-300 mb-3"></i>
                    ...
                    <div className="w-full max-w-lg bg-white rounded-2xl shadow-xl flex flex-col max-h-[90vh] overflow-hidden">
                        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gray-50/50 shrink-0">
                            <h3 className="text-[18px] font-semibold text-[#202223] flex items-center gap-2">
                                <i className="fa-solid fa-building text-[var(--accent)]"></i> Department Details
                            </h3>
                        ...
                        <div className="p-6 overflow-y-auto brass-scroll">
                            <div className="mb-5">
                                <span className="block text-[11px] font-bold uppercase tracking-wider text-gray-500 mb-1">Department Name</span>
                                <div className="text-[18px] font-bold text-gray-900">{selectedDepartment.name}</div>
                            </div>
                            <div className="border-t border-gray-100 pt-5">
                                ...
                                <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 text-gray-700 text-[14px] leading-relaxed min-h-[80px] whitespace-pre-line">
                                    {selectedDepartment.description || <span className="italic text-gray-400">No description provided.</span>}
                                </div>
                            </div>
                        </div>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#0A0E1A]/40 backdrop-blur-sm p-4">
                    <div className="w-full max-w-lg bg-white rounded-2xl shadow-xl flex flex-col max-h-[90vh] overflow-hidden">
                        ...
                        <form onSubmit={handleSubmit} className="flex flex-col overflow-hidden">
                            <div className="p-6 overflow-y-auto brass-scroll">
                                <div className="mb-5">
                                    <label className="block text-[13px] font-semibold text-gray-700 mb-1.5">Department Name *</label>
                                    <input
                                        type="text"
                                        value={data.name}
