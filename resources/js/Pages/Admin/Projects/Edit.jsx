...
export default function Edit({ project, clients = [], managers = [] }) {
    ...
    const handleSubmit = (e) => {
        ...
        if (!data.title || !data.deadline) return Swal.fire({ icon: 'error', title: 'Missing Fields', text: 'Please fill out all required fields (Title, Deadline).', confirmButtonColor: '#111827' });
        ...
        <AdminLayout>
            ...
            <div className="flex flex-col gap-8 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-6">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-5 bg-white p-6 rounded-2xl shadow-[0_2px_10px_-3px_rgba(6,81,237,0.1)] border border-gray-100">
                    <div>
                        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-50 text-amber-600 text-[11px] font-bold uppercase tracking-wider mb-3">
                            <i className="fa-solid fa-pen-ruler"></i> Modification
                        </div>
                        <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-900 tracking-tight">Edit Project</h1>
                        <p className="text-[13.5px] font-semibold text-indigo-600 mt-1">#{project.id} — {project.title}</p>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    <section className="bg-white rounded-2xl border border-gray-200 shadow-[0_4px_20px_-5px_rgba(0,0,0,0.05)] relative">
                        ...
                        <div className="px-6 md:px-8 py-5 border-b border-gray-100 flex items-center gap-3 bg-gradient-to-r from-gray-50/80 to-white rounded-t-2xl">
                            ...
                            <div>
                                ...
                                <p className="text-[12px] text-gray-500 font-medium mt-0.5">Manage client assignment and project manager.</p>
                            </div>
                        </div>
                                ...
                                <div className="mt-7 flex items-center gap-4 rounded-xl border border-indigo-100 bg-indigo-50/50 p-4">
                                    ...
                                    <div className="min-w-0 flex-1">
                                        ...
                                        <p className="text-[13px] text-gray-600 mt-0.5 flex items-center gap-1.5">
                                            ...
                                            Project assigned to this client {selectedManager ? <span className="font-semibold text-gray-700">— Managed by {selectedManager.name}</span> : ""}
                                        </p>
                                    </div>
                                </div>
                            )}
                    ...
                    <section className="bg-white rounded-2xl shadow-[0_4px_20px_-5px_rgba(0,0,0,0.05)] relative border border-gray-200">
                        ...
                        <div className="px-6 md:px-8 py-5 border-b border-gray-100 flex flex-wrap justify-between items-center gap-4 bg-gray-50/40 rounded-t-2xl">
                            <div className="flex items-center gap-4">
                                <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-slate-800 text-white text-[15px] font-black shadow-md shrink-0">
                                ...
                                <div>
                                    ...
                                    {data.title && <p className="text-[13px] text-gray-500 font-medium truncate max-w-xs sm:max-w-md">{data.title}</p>}
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="p-6 md:p-8">
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-x-8 gap-y-7">
                                ...
                                <div className="md:col-span-2 lg:col-span-12 grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-gray-100 pt-6">
                                    <div>
                                        <label className="block text-[12px] font-bold text-gray-600 uppercase tracking-wider mb-2.5">Current Status <span className="text-rose-500">*</span></label>
                                        <select 
                                            value={data.status} 
                                            onChange={e => setData("status", e.target.value)} 
                                            className="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3.5 text-[14px] font-semibold outline-none transition-all focus:border-indigo-500 cursor-pointer shadow-sm"
                                        >
                                    ...
                                    <div>
                                        <label className="flex justify-between text-[12px] font-bold text-gray-600 uppercase tracking-wider mb-2">
                                            <span>Completion Progress</span>
                                            <span className="text-indigo-600 font-extrabold">{data.progress}%</span>
                                        </label>
                                        <input 
                                            type="range" min="0" max="100" 
                                            value={data.progress} 
                                            onChange={e => setData("progress", e.target.value)} 
                                            className="w-full h-2.5 mt-3 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-indigo-600" 
                                        />
                                    </div>
                                </div>
                                ...
                                <div className="md:col-span-2 lg:col-span-12 grid grid-cols-1 md:grid-cols-2 gap-6 border-t border-gray-100 pt-6">
                                    <div>
                                        ...
                                        <input 
                                            ...
                                            className="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3.5 text-[13.5px] outline-none transition-all focus:border-indigo-500 shadow-sm" 
                                        />
                                    </div>
                                    <div>
                                        <label className="block text-[12px] font-bold text-gray-600 uppercase tracking-wider mb-2.5">
                        ...
                        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-col sm:flex-row justify-between items-center gap-4">
                            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-x-5 gap-y-2 text-[13px] font-bold text-gray-500">
                                <div className="flex items-center gap-2 bg-emerald-50 px-3 py-1.5 rounded-lg text-emerald-700">
                                    ...
                                    <span>Budget: <span className="text-[15px]">৳{formatTaka(data.budget)}</span></span>
                                </div>
                                {dLeft !== null && (
                                    <div className={`flex items-center gap-2 px-3 py-1.5 rounded-lg ${dLeft <= 7 ? 'bg-rose-50 text-rose-700 animate-pulse' : 'bg-gray-100 text-gray-700'}`}>
